...
module Jekyll
  class MagickImageFile < Jekyll::StaticFile
    ...
    def initialize(site, base, dir, name, preset)
      ...
      @commands = preset.delete('commands')
    end
    ...
    def generate(site)
      ...
      site.config['mini_magick'].each_pair do |name, preset|
        Dir.glob(File.join(preset['source'], "*.{png,jpg,jpeg,gif}")) do |source|
          site.static_files << MagickImageFile.new(site, site.source, preset['destination'], File.basename(source), preset.clone)
        end
      end
    end
  end
